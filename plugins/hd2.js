...
let handler = async (m, { conn, usedPrefix, command }) => {
  try {
    ...
    // التحقق هل الرسالة تحتوي على صورة
    if (!/image/.test(mime)) {
      return m.reply(`🍵 | قم بالرد أو إرسال صورة مع الأمر: *${usedPrefix + command}*`)
    }

    // رسالة انتظار
    m.reply('⏳ | جاري معالجة الصورة، المرجو الانتظار قليلاً...')

    let media = await quoted.download()
    let result = await hdr(media, 4)

    // رسالة النتيجة
    let decoCaption = `🍒 | تم تحسين الصورة بنجاح! ✨`

    await conn.sendFile(m.chat, result, 'hdr.png', decoCaption, m)

  } catch (err) {
    console.log(err)
    m.reply('❌ | حدث خطأ أثناء معالجة الصورة، حاول مرة أخرى لاحقاً.')
  }
}
...
async function getToken() {
  try {
    ...
    throw new Error('خطأ في التوكن: ' + err.message)
  }
}
